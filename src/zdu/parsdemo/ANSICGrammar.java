...
endRule();
    ...
    final Terminal IDENTIFIER = terminal("IDENTIFIER", ExtendedScanner.IDENTIFIER_TOK);
    final Terminal CONSTANT = terminal("CONSTANT", ExtendedScanner.CONSTANT_TOK);
    final Terminal STRING_LITERAL = terminal("STRING_LITERAL", ExtendedScanner.STRING_LITERAL_TOK);
    final Terminal SIZEOF = terminal("SIZEOF", ExtendedScanner.SIZEOF_TOK);
    final Terminal PTR_OP = terminal("PTR_OP", ExtendedScanner.PTR_OP_TOK);
    final Terminal INC_OP = terminal("INC_OP", ExtendedScanner.INC_OP_TOK);
    final Terminal DEC_OP = terminal("DEC_OP", ExtendedScanner.DEC_OP_TOK);
    final Terminal LEFT_OP = terminal("LEFT_OP", ExtendedScanner.LEFT_OP_TOK);
    final Terminal RIGHT_OP = terminal("RIGHT_OP", ExtendedScanner.RIGHT_OP_TOK);
    final Terminal GE_OP = terminal("GE_OP", ExtendedScanner.GE_OP_TOK);
    final Terminal LE_OP = terminal("LE_OP", ExtendedScanner.LE_OP_TOK);
    final Terminal EQ_OP = terminal("EQ_OP", ExtendedScanner.EQ_OP_TOK);
    final Terminal NE_OP = terminal("NE_OP", ExtendedScanner.NE_OP_TOK);
    final Terminal AND_OP = terminal("AND_OP", ExtendedScanner.AND_OP_TOK);
    final Terminal OR_OP = terminal("OR_OP", ExtendedScanner.OR_OP_TOK);

    final Terminal MUL_ASSIGN = terminal("MUL_ASSIGN", ExtendedScanner.MUL_ASSIGN_TOK);
    final Terminal DIV_ASSIGN = terminal("DIV_ASSIGN", ExtendedScanner.DIV_ASSIGN_TOK);
    final Terminal MOD_ASSIGN = terminal("MOD_ASSIGN", ExtendedScanner.MOD_ASSIGN_TOK);
    final Terminal ADD_ASSIGN = terminal("ADD_ASSIGN", ExtendedScanner.ADD_ASSIGN_TOK);
    final Terminal SUB_ASSIGN = terminal("SUB_ASSIGN", ExtendedScanner.SUB_ASSIGN_TOK);
    final Terminal LEFT_ASSIGN = terminal("LEFT_ASSIGN", ExtendedScanner.LEFT_ASSIGN_TOK);
    final Terminal RIGHT_ASSIGN = terminal("RIGHT_ASSIGN", ExtendedScanner.RIGHT_ASSIGN_TOK);
    final Terminal AND_ASSIGN = terminal("AND_ASSIGN", ExtendedScanner.AND_ASSIGN_TOK);
    final Terminal XOR_ASSIGN = terminal("XOR_ASSIGN", ExtendedScanner.XOR_ASSIGN_TOK);
    final Terminal OR_ASSIGN = terminal("OR_ASSIGN", ExtendedScanner.OR_ASSIGN_TOK);

    final Terminal TYPEDEF = terminal("TYPEDEF", ExtendedScanner.TYPEDEF_TOK);
    final Terminal EXTERN = terminal("EXTERN", ExtendedScanner.EXTERN_TOK);
    final Terminal STATIC = terminal("STATIC", ExtendedScanner.STATIC_TOK);
    final Terminal AUTO = terminal("AUTO", ExtendedScanner.AUTO_TOK);
    final Terminal REGISTER = terminal("REGISTER", ExtendedScanner.REGISTER_TOK);

    final Terminal VOID = terminal(" VOID", ExtendedScanner.VOID_TOK);
    final Terminal CHAR = terminal("CHAR", ExtendedScanner.CHAR_TOK);
    final Terminal SHORT = terminal("SHORT", ExtendedScanner.SHORT_TOK);
    final Terminal INT = terminal("INT", ExtendedScanner.INT_TOK);
    final Terminal LONG = terminal("LONG", ExtendedScanner.LONG_TOK);
    final Terminal FLOAT = terminal("FLOAT", ExtendedScanner.FLOAT_TOK);
    final Terminal DOUBLE = terminal("DOUBLE", ExtendedScanner.DOUBLE_TOK);
    final Terminal SIGNED = terminal("SIGNED", ExtendedScanner.SIGNED_TOK);
    final Terminal UNSIGNED = terminal("UNSIGNED", ExtendedScanner.UNSIGNED_TOK);
    final Terminal TYPE_NAME = terminal("TYPE_NAME", ExtendedScanner.TYPE_NAME_TOK);

    final Terminal STRUCT = terminal("STRUCT", ExtendedScanner.STRUCT_TOK);
    final Terminal UNION = terminal("UNION", ExtendedScanner.UNION_TOK);

    final Terminal ENUM = terminal("ENUM", ExtendedScanner.ENUM_TOK);

    final Terminal CONST = terminal("CONST", ExtendedScanner.CONST_TOK);
    final Terminal VOLATILE = terminal("VOLATILE", ExtendedScanner.VOLATILE_TOK);
    final Terminal ELLIPSIS = terminal("ELLIPSIS", ExtendedScanner.ELLIPSIS_TOK);
    final Terminal CASE = terminal("CASE", ExtendedScanner.CASE_TOK);
    final Terminal DEFAULT = terminal("DEFAULT", ExtendedScanner.DEFAULT_TOK);

    final Terminal IF = terminal("IF", ExtendedScanner.IF_TOK);
    final Terminal ELSE = terminal("ELSE", ExtendedScanner.ELSE_TOK);
    final Terminal SWITCH = terminal("SWITCH", ExtendedScanner.SWITCH_TOK);

    final Terminal WHILE = terminal("WHILE", ExtendedScanner.WHILE_TOK);
    final Terminal DO = terminal("DO", ExtendedScanner.DO_TOK);
    final Terminal FOR = terminal("for", ExtendedScanner.for_TOK);

    final Terminal GOTO = terminal("GOTO", ExtendedScanner.GOTO_TOK);
    final Terminal BREAK = terminal("BREAK", ExtendedScanner.BREAK_TOK);
    final Terminal RETURN= terminal("RETURN", ExtendedScanner.RETURN_TOK);
    final Terminal CONTINUE = terminal("CONTINUE", ExtendedScanner.CONTINUE_TOK);
