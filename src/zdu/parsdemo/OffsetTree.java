...
Last Update Time-stamp: "97/06/27 20:36:33 umrigar"
...
class OffsetTree extends Tree {
  ...
  private void offsetToSep() {
    if (!isLeaf()) {
      ...
      firstKid.offset = 0;
      for (OffsetTree t = (OffsetTree) firstKid.nextSib(); t != null; t = (OffsetTree) (t.nextSib())) {
        int oldOff = t.offset;
        t.offset -= off;
        off = oldOff;
      }
  ...
  private void sepToOffset() {
    ...
    if (nKids > 0) {
      OffsetTree firstKid = (OffsetTree) (kid());
      int off = 0;
      for (OffsetTree kid = (OffsetTree) firstKid.nextSib(); kid != null; kid = (OffsetTree) kid.nextSib()) {
        kid.offset += off;
        off = kid.offset;
      }
      int rootOffset = (nKids % 2 == 0)
          ? (((OffsetTree) kid(nKids / 2 - 1)).offset +
              ((OffsetTree) kid(nKids / 2)).offset) / 2
          : ((OffsetTree) kid(nKids / 2)).offset;
      for (OffsetTree kid = firstKid; kid != null; kid = (OffsetTree) kid.nextSib()) {
  ...
  private boolean computeContours() {
    ...
    if (nKids1 > 0) {
      ...
      for (contourN = LEFT, startKidN = 0, kidNInc = 1; contourN <= RIGHT; contourN++, startKidN = (nKids1
          - 1), kidNInc = -1) {
        int h;
        if (contourN == LEFT) {
          // Compute max. ht. of all subtrees.
          int kidsHMax = 0;
          for (OffsetTree kid = (OffsetTree) kid(); kid != null; kid = (OffsetTree) kid.nextSib()) {
  ...
  private int leftSep(OffsetTree kid) {
    ...
    for (int i = 0; i < h; i++) {// for each level of this.
      while (t != null && t.contours[RIGHT].length <= i) {
        ...
        t = (OffsetTree) t.nextSib(-1);
      }
      if (t == null)
        break; // kid higher than anything to its left.
  ...
  private int rightSep(OffsetTree kid) {
    ...
    separate: for (int i = 0; i < h; i++) {// for each level of this.
      while (t != null && t.contours[LEFT].length <= i) {
        t = (OffsetTree) t.nextSib();
        if (t == null)
          break separate; // kid higher than anything to right.
        tRootOffset += t.offset;
      }
      // t does have a node at level i; compute offsets at level i.
  ...
  private int fullSep(OffsetTree kid) {
    ...
    separate: for (int i = 0; true; i++) {
      ...
      while (tLeft != null && tLeft.contours[RIGHT].length <= i) {
        leftRootOffset -= tLeft.offset;
        tLeft = (OffsetTree) tLeft.nextSib(-1);
      }
