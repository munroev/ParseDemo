...
class Tree {
  ...
  Tree(Object info, Tree kids[]) {
    ...
    for (int i = 0; i < n - 1; i++) {
      kids[i].flags &= ~IS_LAST_SIB;
      kids[i].sibs = kids[i + 1];
    }
    kids[n - 1].flags |= IS_LAST_SIB;
    kids[n - 1].sibs = this;
  }
  ...
  public int nKids() {
    ...
    if ((flags & IS_LEAF) == 0) {
      n = 1;
      for (Tree t = kids; (t.flags & IS_LAST_SIB) == 0; t = t.sibs)
        n++;
    }
    return n;
  }
  ...
  public void addKid(Tree kid, int k) {
    if ((flags & IS_LEAF) != 0) { // No existing kid.
      ...
      int i;
      Tree t;
      for (i = 0, t = kids; i < k && (t.flags & IS_LAST_SIB) == 0; i++, t = t.sibs) {
      }
      // Insert after t.
      kid.sibs = t.sibs;
      kid.flags &= ~IS_LAST_SIB;
      kid.flags |= (t.flags & IS_LAST_SIB);
      t.sibs = kid;
      t.flags &= ~IS_LAST_SIB;
    }
  }
  ...
  public Tree rmKid(Tree kid) {
    ...
    if ((flags & IS_LEAF) == 0) {
      if (kids == kid) { // Kid is first kid.
        ...
        Tree t;
        Tree t0; // Lags t.
        for (t0 = kids, t = kids.sibs; t != kid && (t.flags & IS_LAST_SIB) == 0; t0 = t, t = t.sibs) {
        }
        if (t == kid) {
  ...
  public Tree rmKid(int n) {
    ...
    if ((flags & IS_LEAF) == 0) {
      if (n == 0) { // Kid is first kid.
        ...
        for (t0 = kids, t = t0.sibs, i = 1; i != n && (t.flags & IS_LAST_SIB) == 0; t0 = t, t = t.sibs, i++) {
        }
        if (i == n) {
          t0.flags |= (t.flags & IS_LAST_SIB);
          t0.sibs = t.sibs;
  ...
  public Tree kid(int k) {
    ...
    if ((flags & IS_LEAF) == 0) {
      Tree t;
      int i;
      for (t = kids, i = 0; i < k && (t.flags & IS_LAST_SIB) == 0; i++, t = t.sibs) {
      }
      if (i == k)
        kid = t;
    }
  ...
  public int getKidN(Tree kid) {
    ...
    if ((flags & IS_LEAF) == 0) {
      Tree t;
      for (t = kids, kidN = 0; t != kid && (t.flags & IS_LAST_SIB) == 0; kidN++, t = t.sibs) {
      }
      if (t != kid)
        kidN = -1;
    }
  ...
  public String treeString(int indent) {
    return appendToStringBuffer(new StringBuffer(), indent, INDENT_INC).toString();
  }
  ...
  public String treeString(int indent, int indentInc) {
    return appendToStringBuffer(new StringBuffer(), indent, indentInc).toString();
  }
